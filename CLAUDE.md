...
- [x] 팀 관리 기능 — 팀 생성(팀장)은 **프리미엄 전용**, 팀원 연결은 **기본 플랜 이상만** 가능
      ...
      **팀원 한도 대여**: 팀원이 본인 월 사용량(문자/스크립트·성향분석/PDF분석/콘텐츠/뉴스레터)을
      다 쓰면, 소속 팀장의 남은 한도에서 자동으로 대신 차감됨(`lib/subscription/usage.ts`의
      `reserveUsage()`). 팀장 본인 한도까지 소진되면 그때는 정상적으로 한도 초과 처리.
