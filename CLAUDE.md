...
1. **[검증 대기] 일정 알림(푸시) 안 오는 문제 — 2026-07-14~15에 걸쳐 원인 6개를 순서대로 발견·수정.**
   ...
   - 위 6가지 모두 수정 완료 상태로 실사용자가 재테스트 예정 — 결과 확인 대기. 기존 테스트
     일정은 이미 알림 구간이 지나 재발송되지 않으니, 재테스트 시 반드시 새 일정으로 할 것.
     여전히 안 오면 다음 확인 대상은 Vercel Cron Jobs 페이지의 `/api/cron/push-notify` 실행
     로그(View Logs) — 크론이 실제로 도는지, 각 실행에서 `sent`/`notified` 값이 몇인지 확인 필요.
