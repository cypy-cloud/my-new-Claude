...
[PERSUASIVE]
(설득력 있는 버전 - 400~700자 분량으로 충분히 길고 설득력 있게 작성. 다음 구조를 반드시 따를 것:
 1) 호기심을 자극하는 후킹 오프닝 한 줄 (예: "혹시 ○○하신 적 있으신가요?", "최근 많은 분들이 모르고 놓치는 보장이 있어요" 등)
 2) 고객(연령대·직업 등)이 자신의 이야기처럼 이입할 수 있는 구체적이고 현실적인 상황·사례를 2~3문장으로 묘사 (예: "직장 동료 중 한 분이 건강검진에서...", "최근 비슷한 연령대 고객님들이..." 처럼 실제 있을 법한 에피소드 형태로, 과장이나 공포 조장 없이 공감을 일으키는 톤으로)
 3) "특별 보장 내용(후킹 포인트)"에 구체적인 내용이 입력된 경우, 그 내용을 반드시 메시지 본문에 자연스럽게 녹여 넣어 고객이 "이게 뭐지?" 궁금해할 만한 구체적 디테일(금액, 조건, 기간 등)을 직접 언급할 것. 입력된 내용이 없다면 일반적인 보장 공백/누락 가능성을 언급해 궁금증을 유발할 것.
 4) 혜택 중심 설명 (왜 지금 알아야 하는지)
 5) 답장/연락을 유도하는 명확한 행동 유도 문구로 마무리 (예: "궁금하신 점 있으시면 편하게 답장 주세요", "자세한 보장 내용 알려드릴게요" 등 — 직접 전화하라는 압박이 아니라 답장을 유도하는 톤)
 이 섹션은 반드시 후킹 오프닝, 공감 상황 묘사, 행동 유도 마무리를 모두 포함해야 하며, 누락 시 무효로 간주됩니다.)
