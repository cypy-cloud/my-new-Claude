...
- **블로그·SNS 콘텐츠 생성**: 블로그 본문, 인스타그램, 페이스북, 카카오채널, 해시태그를 한 번에 AI 생성
- **뉴스레터 생성**: 8개 섹션(인사말·시사이슈·체크포인트·CTA·카카오요약)으로 구성된 월간 뉴스레터 AI 생성 및 TXT 다운로드
- **업무 캘린더**: 고객 후속연락·미팅·갱신·생일 일정을 오늘/주간/월간 뷰로 관리, 구글 캘린더 연동 준비(gcal_event_id 컬럼)
- **고객 관리**: 고객 정보 등록·조회·상호작용 이력 관리, 캘린더 일정 연동
- **팀 관리**: 팀 계정 생성, 멤버 초대·역할 관리
- **결과물 보관함**: 생성된 모든 결과물(문자/스크립트/PDF분석/뉴스레터/콘텐츠)을 저장하고 다운로드/검색 가능
...
`supabase/migrations/` 폴더에 `001`부터 `040`까지 순서가 매겨진 SQL 마이그레이션 파일이 있습니다. Supabase 대시보드 → SQL Editor에서 **파일명 순서대로** 하나씩 실행합니다.

```
supabase/migrations/001_initial_schema.sql
supabase/migrations/002_usage_records.sql
...
supabase/migrations/039_tasks.sql
supabase/migrations/040_security_hardening.sql
```
