...
6. CTA(고객 행동 유도 문구)에 "그동안 안부 문자로만 연락드려 죄송합니다" 같은 사과 표현을
   절대 넣지 마세요 — 유용한 정보를 전달하는 뉴스레터에 불필요한 사과이며, 자신감 없어 보입니다
7. 마크다운 기호(#, ##, **, - 등)를 절대 쓰지 마세요 — 이 뉴스레터는 이미지로 변환되어
   그대로 노출되므로, 소제목도 기호 없이 순수 텍스트 한 줄로만 작성하세요
...
[ISSUE_1]
(핵심 이슈 1 소제목 — 기호 없이 텍스트 한 줄)
(이슈 설명 — 고객에게 왜 중요한지 중심으로, 200~400자)

[ISSUE_2]
(핵심 이슈 2 소제목 — 기호 없이 텍스트 한 줄)
(이슈 설명 — 실질적인 정보 위주, 200~400자)

[ISSUE_3]
(핵심 이슈 3 소제목 — 기호 없이 텍스트 한 줄)
(이슈 설명 — 행동 촉구 내용 포함, 200~400자)

[CHECK_POINTS]
(고객이 지금 당장 확인해야 할 보험 점검 포인트 정확히 3가지만. 항목별 하위 설명 문단
없이, 각각 "✓ "로 시작하는 한 줄짜리 문장으로 간결하고 강력하게 후킹되도록 작성)

[CTA]
(고객 행동 유도 문구 — 상담 예약, 연락 요청 등 자연스러운 CTA, 2~3문장. 사과 표현으로 시작하지 말 것)
