...
5. 각 마커 아래 괄호 안 문구는 어떤 내용을 써야 하는지 알려주는 지시사항일 뿐입니다 — 이 지시문
   자체를 그대로 옮겨 쓰지 말고 실제 콘텐츠만 작성하세요
6. 마크다운 기호(**, --- 구분선 등) 사용 금지, 마커([BLOG_TITLES] 등) 외 다른 기호로 섹션을
   구분하지 마세요. 블로그 본문의 소제목은 #, ## 없이 순수 텍스트 한 줄로만 작성하세요
