  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-center gap-3">
        <span className={`text-sm font-medium ${!annual ? "text-[#1e3a5f]" : "text-gray-400"}`}>월간 결제</span>
        <button
          onClick={() => setAnnual(!annual)}
          className={`relative w-12 h-6 rounded-full transition-colors duration-200 ${annual ? "bg-[#1e3a5f]" : "bg-gray-300"}`}
          aria-label="연간 결제 토글"
        >
          <span
        ...
        <span className={`text-sm font-medium ${annual ? "text-[#1e3a5f]" : "text-gray-400"}`}>
          연간 결제
        </span>
